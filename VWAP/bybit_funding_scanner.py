...
def get_candle_width(symbol):
    """
    Tính toán candle width trung bình (5m/24h) từ candle_width_stat.py
    """
    try:
        ...
        # print(f"Lỗi tính candle width cho {symbol}: {e}")
        return 0.0
...
def prepare_display_data(items, target_map):
...
def process_short_interval_signals(data_list, target_map):
    """Xử lý tín hiệu 1h/2h, hiển thị bảng và gửi Telegram."""
    short_interval_data = []
    for item in data_list:
        if item['interval'] in [60, 120]:
            item['daily_change'] = None
            short_interval_data.append(item)
            
    if not short_interval_data:
        return
    ...
    final_display_short = prepare_display_data(top_10_short, target_map)
    ...
    try:
        if final_display_short:
            msg_lines.append(f"🔥 *HIGH FREQ FUNDING (1h/2h)*")
            msg_lines.append("```")
        ...
        print(f"Error preparing Telegram (Short): {e}")
    return msg_lines

def process_long_interval_signals(data_list, target_map, top_200_symbols):
    """Xử lý tín hiệu 4h/8h, lọc Top 200 CMC, kiểm tra history, hiển thị bảng và gửi Telegram."""
    long_interval_data = []
    for item in data_list:
        if item['interval'] in [240, 480]:
            sym = item['symbol']
            base = sym.split('/')[0]
    ...
    final_display_long = prepare_display_data(top_10_long, target_map)
    ...
    try:
        if final_display_long:
            msg_lines.append(f"📈 *4H/8H FUNDING (>10% & Hist <0.05%)*")
            msg_lines.append("```")
        ...
        print(f"Error preparing Telegram (Long): {e}")
    return msg_lines

def check_volume_condition(symbol, timeframe, multiplier=2, avg_period=10, lookback=1):
    """
    Kiểm tra điều kiện Volume Pump: Xét lookback cây nến đã đóng gần đây nhất, 
    chỉ cần 1 cây thỏa điều kiện (Volume > multiplier * Volume trung bình của avg_period cây trước đó).
    Trả về: (is_pump, max_surge_pct, price_surge_pct)
    """
    try:
        limit = avg_period + lookback + 1
        ...
        # Duyệt qua các cây nến trong lookback (từ mới nhất đến cũ nhất)
        for i in range(1, lookback + 1):
            target_idx = -(i + 1)
            target_candle = ohlcv[target_idx]
            target_open = target_candle[1]
            target_close = target_candle[4]
            target_volume = target_candle[5]
            
            # 10 nến trước target_candle
            prev_candles = ohlcv[target_idx - avg_period : target_idx]
            avg_volume = sum(c[5] for c in prev_candles) / len(prev_candles)
            
            surge_pct = (target_volume / avg_volume - 1) * 100 if avg_volume > 0 else 0
            price_surge = (target_close / target_open - 1) * 100 if target_open > 0 else 0
            
            if target_volume > (multiplier * avg_volume):
                is_any_pump = True
                # Lấy surge của cây có volume đột biến nhất
                if surge_pct > max_surge_pct:
                    max_surge_pct = surge_pct
                    best_price_surge = price_surge
                    
        return is_any_pump, max_surge_pct, best_price_surge
    except Exception as e:
        return False, 0, 0

def process_volume_pump_signals(data_list, target_map):
    """
    Quét tín hiệu Kéo Volume (Volume Pump) trên cả khung 1D và 1H.
    Khung 1H xét 3 cây nến gần nhất.
    """
    vp_data = []
    print("Scanning for Volume Pump signals (1D + 1H Lookback 3)...")
    
    for item in data_list:
        sym = item['symbol']
        
        # Conditions: 1D volume surge (yesterday) AND 1H volume surge (any of last 3)
        is_pump_1d, surge_1d, p_surge_1d = check_volume_condition(sym, '1d', lookback=1)
        is_pump_1h, surge_1h, p_surge_1h = check_volume_condition(sym, '1h', lookback=3)
        
        if is_pump_1d and is_pump_1h:
            item['daily_change'] = get_daily_change(sym)
            item['vol_surge_1d'] = surge_1d
            item['vol_surge_1h'] = surge_1h
            item['price_surge_1d'] = p_surge_1d
            item['price_surge_1h'] = p_surge_1h
            vp_data.append(item)
            
    if not vp_data:
        print("No Volume Pump signals found.")
        return
        
    vp_data.sort(key=lambda x: abs(x['funding_rate']), reverse=True)
    top_vp = vp_data[:10]
    
    final_display_vp = prepare_display_data(top_vp, target_map)
    
    # Custom display for Volume Pump with 4 extra columns
    print("\n" + "="*185)
    print(f"VOLUME PUMP SIGNALS (1D + 1H Surge x2) - {datetime.now().strftime('%H:%M:%S')}")
    print("="*185)
    print(f"{'Name':<15} {'Price':<12} {'Funding Rate % (Hist)':<40} {'1h':<8} {'1D':<8} {'1D Vol%':<12} {'1H Vol%':<12} {'1D Prc%':<12} {'1H Prc%':<12} {'Market Cap':<15}")
    print("-" * 185)
    
    for disp_item in final_display_vp:
        # Find original item to get vol/price surge values
        orig_item = next(x for x in top_vp if x['symbol'] == disp_item['Name'])
        
        current_rate = disp_item['Funding Rate %']
        hist = disp_item['History']
        cur_str = f"{current_rate*100:+.4f}%"
        hist_str = f" ({', '.join([f'{r*100:+.4f}%' for r in hist])})" if hist else ""
        full_rate_str = cur_str + hist_str
        
        p_price = f"${disp_item['Price']:.4f}" if disp_item['Price'] < 100 else f"${disp_item['Price']:.2f}"
        c1h = f"{disp_item['1h Change']:+.2f}%"
        c1d = f"{disp_item['1D Change']:+.2f}%" if disp_item['1D Change'] != 0 else "N/A"
        
        v1d_str = f"{orig_item['vol_surge_1d']:+.2f}%"
        v1h_str = f"{orig_item['vol_surge_1h']:+.2f}%"
        p1d_str = f"{orig_item['price_surge_1d']:+.2f}%"
        p1h_str = f"{orig_item['price_surge_1h']:+.2f}%"
        
        mc = disp_item['MarketCap']
        if mc >= 1_000_000_000: mcap_str = f"${mc/1_000_000_000:.2f}B"
        elif mc >= 1_000_000: mcap_str = f"${mc/1_000_000:.2f}M"
        else: mcap_str = f"${mc:,.0f}" if mc > 0 else "N/A"
        
        print(f"{disp_item['Name']:<15} {p_price:<12} {full_rate_str:<40} {c1h:<8} {c1d:<8} {v1d_str:<12} {v1h_str:<12} {p1d_str:<12} {p1h_str:<12} {mcap_str:<15}")
    
    # --- PREPARE TELEGRAM CONTENT ---
    msg_lines = []
    try:
        if final_display_vp:
            msg_lines.append(f"📊 *VOLUME PUMP DETECTED (1D+1H)*")
            msg_lines.append("```")
            msg_lines.append(f"{'Name':<8} {'1D Vol%':<10} {'1H Vol%':<10} {'1D Pr%':<8} {'1H Pr%':<8}")
            msg_lines.append("-" * 48)
            
            for disp_item in final_display_vp:
                orig_item = next(x for x in top_vp if x['symbol'] == disp_item['Name'])
                name = disp_item['Name'].split('/')[0]
                
                v1d_short = f"{orig_item['vol_surge_1d']:+.0f}%"
                v1h_short = f"{orig_item['vol_surge_1h']:+.0f}%"
                p1d_short = f"{orig_item['price_surge_1d']:+.1f}%"
                p1h_short = f"{orig_item['price_surge_1h']:+.1f}%"
                
                row_str = f"{name:<8} {v1d_short:<10} {v1h_short:<10} {p1d_short:<8} {p1h_short:<8}"
                msg_lines.append(row_str)
            msg_lines.append("```")
    except Exception as e:
        print(f"Error preparing Telegram (Volume): {e}")
    return msg_lines
...
def main():
    ...
    while True:
        try:
            ...
            # 3. Process Signals
            s_msg = process_short_interval_signals(data_list, target_map)
            l_msg = process_long_interval_signals(data_list, target_map, top_200_symbols)
            v_msg = process_volume_pump_signals(data_list, target_map)
            
            # --- Combined Telegram Notification ---
            total_msg_lines = []
            if s_msg or l_msg or v_msg:
                total_msg_lines.append(f"🔔 *BYBIT SCANNER REPORT* - {datetime.now().strftime('%H:%M')}")
                if s_msg: total_msg_lines.extend(["", *s_msg])
                if l_msg: total_msg_lines.extend(["", *l_msg])
                if v_msg: total_msg_lines.extend(["", *v_msg])
                
                combined_msg = "\n".join(total_msg_lines)
                send_telegram_message(combined_msg)
            
            # --- Schedule: Wait for next 30 minutes ---
            now = datetime.now()
            # Calculate next 30 minute interval (00, 30)
            # Add a small buffer (5s) to ensure we move past the current minute if we finish exactly at minute 00/30
            minutes_to_add = 30 - (now.minute % 30)
            next_run = (now + timedelta(minutes=minutes_to_add)).replace(second=0, microsecond=0)
            
            # If next_run is too close, push to the one after it
            if (next_run - now).total_seconds() < 10:
