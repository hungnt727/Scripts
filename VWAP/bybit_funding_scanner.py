...
def check_low_funding_history(symbol, days=10, threshold=0.01):
    """
        ...
        try:
            ...
            for item in data_list:
                ...
                if interval_val in [240, 480]:  # 4h or 8h
                    # Check daily change > 20% AND Funding history < 0.1% for last 10 days
                    daily_chg = get_daily_change(item['symbol'])
                    if daily_chg is not None and daily_chg > 20:
                        if check_low_funding_history(item['symbol']):
            ...
            display_table(final_display_long, "TOP 10 COINS - 4H/8H FUNDING - Daily >20% & Hist <0.01%")
            ...
            try:
                ...
                if final_display_long:
                    msg_lines = []
                    msg_lines.append(f"📈 *4H/8H FUNDING (>20% & Hist <0.1%)* - {datetime.now().strftime('%H:%M')}")
                    msg_lines.append("```")
