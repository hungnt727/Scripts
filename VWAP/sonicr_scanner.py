...
def check_sonicr_signals(df, symbol, timeframe):
    ...
    for i in range(LOOKBACK_CANDLES):
        ...
        for setup in SONICR_SETUP_CONFIGS:
            ...
            cross_distance = 0
            for j in range(len(df) - 1, curr_idx, -1):
                idx_j = -1 - (len(df) - 1 - j)
                if abs(idx_j) > len(df): break
                c_j = df.iloc[idx_j]
                c_prev = df.iloc[idx_j - 1] if abs(idx_j - 1) <= len(df) else None
                if c_prev is None: break
                ...
                if (c_j['ema_rsi_200'] > c_j['ema_rsi_610'] and c_prev['ema_rsi_200'] <= c_prev['ema_rsi_610']) or \
                   (c_j['ema_rsi_200'] < c_j['ema_rsi_610'] and c_prev['ema_rsi_200'] >= c_prev['ema_rsi_610']):
                    cross_distance = abs(curr_idx - idx_j)
                    break
    ...
    while True:
        ...
        for config in SONICR_TIMEFRAME_CONFIGS:
            ...
            for coin in top_coins:
                ...
                if df is not None:
                    found = check_sonicr_signals(df, coin, timeframe)
                    if found:
