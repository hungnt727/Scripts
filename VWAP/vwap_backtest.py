...
def run_backtest():
    ...
    def print_stats(df, title="ALL TIMEFRAMES"):
        ...
        win_soft_sl = df[(df['reason'] == 'SL') & (df['pnl'] > 0)]
        
        loss_vwap = df[(df['reason'] == 'SL') & (df['pnl'] <= 0)]
        loss_technical = df[(df['reason'].isin(['SL_BE', 'TP1+TP2', 'TP2'])) & (df['pnl'] <= 0)]
        ...
        print(f"Số lệnh thắng khác (Soft SL > Entry): {len(win_soft_sl)}")
        print(f"Tổng số lệnh thua: {len(loss_trades)}")
        print(f"Số lệnh thua đảo ngược vwap (Soft SL): {len(loss_vwap)}")
        print(f"Số lệnh thua kỹ thuật (Band 1 < Entry): {len(loss_technical)}")
        ...
        print(f"Số cây nến từ trung bình lúc mở lệnh tới lúc đóng lệnh Lời: {avg_win_duration:.1f}")
        print(f"Số cây nến từ trung bình lúc mở lệnh tới lúc đóng lệnh Lỗ: {avg_loss_duration:.1f}")
